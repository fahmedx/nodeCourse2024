...
module.exports = class PetController {
    ...
    static async removePetById(req,res){
        ...
        if(pet.user._id.toString() !== user._id.toString()){
            res.status(422).json({
                message: "Ocorreu um erro no processamento de sua solicitação, por gentileza tentar mais tarde."
            })
            return
        }
    ...
    static async schedule(req,res){
        ...
        if(pet.user._id.equals(user._id)){
            res.status(422).json({
                message: "Você não pode agendar uma visíta para seu próprio Pet."
            })
            return
        }
    ...
    static async concludeAdoption(req,res){
        ...
        res.status(200).json({message: `Parabéns, o ciclo de adoção está completo!`})
    }
}